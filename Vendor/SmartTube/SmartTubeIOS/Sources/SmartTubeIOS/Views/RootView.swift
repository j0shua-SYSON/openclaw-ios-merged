    ...
    var body: some View {
        ...
        .landscapePlayerCover(item: tosFullScreenBinding, dismissStore: tosState) { video in
            TOSPlayerView(video: video, api: api) {
                ...
                //
                // tosState.currentVideo, not the captured `video`: swipe navigation
                // swaps the vm under the presented cover (LandscapePresenter never
                // updates rootView while it is on screen), so `video` is the item
                // from presentation time. Using it would mark and re-open the wrong
                // video while the actually-failing one keeps looping.
                let failing = tosState.currentVideo ?? video
                tosState.markFallback(videoId: failing.id)
                playerRouter.open(video: failing, api: api)
            }
        }
