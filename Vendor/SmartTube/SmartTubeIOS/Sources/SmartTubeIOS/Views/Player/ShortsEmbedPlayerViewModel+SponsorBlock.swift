...
extension ShortsEmbedPlayerViewModel {
    ...
    func fetchSponsorSegments() async {
        ...
        // This VM persists across shorts (unlike TOSPlayerViewModel, one instance per
        // video, from which this file was ported): after every await, re-check that
        // the user hasn't swiped on — installing segments for the previous short makes
        // the current one auto-skip at the wrong timestamps.
        ...
        if let cachedSegments = cached.sponsorSegments {
            ...
            Task(priority: .background) { [weak self] in
                ...
                await MainActor.run {
                    // Untracked task, can land seconds after a swipe — same guard.
                    guard self.videoId == videoId else { return }
