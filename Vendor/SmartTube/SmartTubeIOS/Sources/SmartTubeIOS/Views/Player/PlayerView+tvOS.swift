...
extension PlayerView {
    ...
    enum MoreMenuRow: Hashable {
        case speed, like, dislike, sleepTimer, audioOnly, queueShuffle, captions, audioTrack, description, comments, statsForNerds, cancel
    }
    ...
    var tvProgressBar: some View {
        VStack(spacing: 6) {
            ...
            GeometryReader { geo in
                ...
                // Pass this track's 40 pt inset — the markers' default (20 pt, the iOS
                // track inset) drew every tick/segment against the wrong geometry here.
                .overlay(sponsorBlockMarkers(hPad: hPad))
                .overlay(chapterMarkers(hPad: hPad))
            }
            .frame(height: 36)
