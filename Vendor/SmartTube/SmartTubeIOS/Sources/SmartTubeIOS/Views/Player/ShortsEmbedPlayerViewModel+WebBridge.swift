...
extension ShortsEmbedPlayerViewModel {
    ...
    func handleScriptMessage(_ body: String, frameInfo: WKFrameInfo) {
        ...
        // STALE-FRAME GUARD (all message types, same mechanism as the original
        // "ready"-only guard below): after loadShort() swaps the iframe src, the
        // OUTGOING document keeps posting tick/stateChange/error for the ~100-500ms
        // until the new document commits (#275 confirmed cross-load messages). An
        // old frame's terminal tick (state=ended) re-fired handleShortEnded and
        // skipped the freshly-loaded short; an old frame's late "error" marked the
        // healthy new short as failed; old ticks overwrote currentTime with the
        // previous video's position. The posting frame's embed URL encodes its
        // videoId — drop anything that doesn't match the video this VM is on now.
        if let frameVideoId = frameInfo.request.url.flatMap(ShortsEmbedURL.videoId(fromEmbedURL:)),
           frameVideoId != videoId {
            shortsLog.notice("[\(self.logTag, privacy: .public)] [ytCallback] \(type, privacy: .public) DISCARDED — stale frame for videoId=\(frameVideoId, privacy: .public), current videoId=\(self.videoId, privacy: .public)")
            return
        }
