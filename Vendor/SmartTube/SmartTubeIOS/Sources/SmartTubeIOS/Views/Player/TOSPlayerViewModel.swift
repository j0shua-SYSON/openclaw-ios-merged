...
final class TOSPlayerViewModel: NSObject {
    ...
    /// Last values applied via `updateAuthToken(_:)` / `updateSAPISID(_:)`. Two jobs:
    /// (1) make those methods no-ops when the value is unchanged — TOSPlayerView.onAppear
    /// re-fires on every expand from the mini-player, and unconditionally re-clearing the
