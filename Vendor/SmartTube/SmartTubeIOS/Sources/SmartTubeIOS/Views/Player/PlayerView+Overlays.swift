...
extension PlayerView {
        ...
        Button {
            ...
            HStack {
                ...
                Text(store.settings.playbackSpeed == 1.0 ? "Normal"
                     : "\(store.settings.playbackSpeed, specifier: "%g")×")
                    .foregroundStyle(.secondary)
            }
            .padding()
    ...
    @ViewBuilder private var moreMenuStatsForNerdsRow: some View {
        ...
        #if os(tvOS)
        // .statsForNerds, not .cancel (copy-paste from the row below): keyed to
        // .cancel this row highlighted together with Cancel and — lacking its own
        // .focused binding and moreMenuVisibleRows entry — was unreachable by D-pad.
