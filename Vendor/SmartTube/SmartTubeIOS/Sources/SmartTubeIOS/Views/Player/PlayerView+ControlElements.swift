...
struct PlayerControlsOverlay: View {
    ...
    var body: some View {
        VStack {
            ...
            HStack(spacing: 40) {
                #if os(tvOS)
                seekButton(symbol: Self.seekSymbol("gobackward", seconds: store.settings.seekBackSeconds),
                ...
                playPauseButton
                #if os(tvOS)
                seekButton(symbol: Self.seekSymbol("goforward", seconds: store.settings.seekForwardSeconds),
...
extension PlayerControlsOverlay {
    ...
    /// SF Symbols only ships numbered gobackward/goforward variants for these values,
    /// but Settings allows others (20 in the tvOS picker; arbitrary multiples of 5 via
    /// the macOS stepper). A non-existent symbol name renders an EMPTY image — an
    /// invisible-but-tappable seek button — so fall back to the un-numbered arrow.
    static let sfNumberedSeekVariants: Set<Int> = [5, 10, 15, 30, 45, 60, 75, 90]
    static func seekSymbol(_ base: String, seconds: Int) -> String {
        sfNumberedSeekVariants.contains(seconds) ? "\(base).\(seconds)" : base
    }
    ...
    // `hPad` must match the horizontal inset of the track these markers overlay:
    // 20 pt on the iOS bar, 40 pt on the tvOS bar — a mismatch shifts every marker.
    func chapterMarkers(hPad: CGFloat = 20) -> some View {
        GeometryReader { geo in
    ...
    // Same hPad contract as chapterMarkers(hPad:) above.
    func sponsorBlockMarkers(hPad: CGFloat = 20) -> some View {
        GeometryReader { geo in
            let trackW = geo.size.width - hPad * 2
...
extension PlayerControlsOverlay {
    ...
    private var speedLabel: String {
        store.settings.playbackSpeed == 1.0 ? "Normal"
            : String(format: "%g", store.settings.playbackSpeed) + "×"
    }
