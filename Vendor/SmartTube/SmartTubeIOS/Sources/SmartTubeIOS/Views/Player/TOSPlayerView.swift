...
public struct TOSPlayerView: View {
    ...
    public var body: some View {
        ...
        .task {
            // Only restore on a fresh session. This view is re-created (and .task
            // re-fires) every time the cover is re-presented — e.g. expanding from
            // the mini-player — but the stored position is only written on stop or
            // video switch, so mid-session it is stale and the seek would yank live
            // playback backwards.
    ...
    private func speedLabel(for speed: Double) -> String {
        speed == 1.0 ? "Normal (1\u{d7})" : String(format: "%g\u{d7}", speed)
    }
    ...
    private func moreButton(vm: TOSPlayerViewModel) -> some View {
        Menu {
            ...
            // vm.videoId, not video.id: `video` is the item captured when the cover was
            // presented and goes stale after swipe navigation swaps the vm underneath.
            if let url = URL(string: "https://www.youtube.com/watch?v=\(vm.videoId)") {
                ShareLink(item: url) {
