                    ...
                    ForEach(displayVideos) { video in
                        ...
                        VideoCardView(video: video, compact: true, currentPlaylistId: playlistId, onSelect: {
                                Task { @MainActor in
                                    ...
                                    // Index into `captured` (the Hide-Shorts-FILTERED list the queue was just
                                    // replaced with), not playlistIndex (the UNFILTERED position): with Shorts
                                    // hidden the indices shift and videoAt(startIdx) played the wrong video.
                                    let startIdx = captured.firstIndex(where: { $0.id == video.id }) ?? video.playlistIndex ?? 0
