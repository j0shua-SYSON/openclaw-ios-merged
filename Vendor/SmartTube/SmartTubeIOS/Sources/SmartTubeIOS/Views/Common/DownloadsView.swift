                ...
                List {
                    ForEach(downloadStore.entries.sorted { $0.downloadedAt > $1.downloadedAt }) { entry in
                        DownloadedVideoRow(entry: entry)
                            ...
                            .onTapGesture {
                                // Downloads always play in the native AVPlayer (local file;
                                // the TOS embed can't play it), so this bypasses PlayerRouter —
                                // but must still enforce the router's mutual-exclusivity rule,
                                // or a running TOS mini-player keeps playing under the cover
                                // (double audio, two mini-players).
