    ...
    private func activationView(info: AuthService.ActivationInfo) -> some View {
        ScrollView {
            VStack(spacing: 28) {
                ...
                Button(role: .cancel) {
                    auth.cancelSignIn()
                    // Immediately request a fresh device code (same as the countdown-expiry
                    // path above). The sign-in-starting .task is one-shot per appearance, so
                    // cancelling without re-beginning stranded the sheet on the
                    // "Connecting to Google…" spinner with no way to get a new code.
                    Task { await auth.beginSignIn() }
                } label: {
