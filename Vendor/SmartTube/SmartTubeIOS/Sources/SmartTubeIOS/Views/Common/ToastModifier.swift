...
private struct ToastModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            ...
            // Also arm on appear: the bound value often lives on a persistent view
            // model (e.g. PlaybackViewModel.toastMessage), so a toast that was on
            // screen when the view was torn down (minimize to mini-player) is still
            // non-nil when a new view identity appears — onChange alone never fires
            // for it and the pill would stay on screen forever.
            .onAppear {
    ...
    private func scheduleDismiss(for value: String?) {
        dismissTask?.cancel()
