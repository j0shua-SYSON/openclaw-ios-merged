    ...
    private func reloadWKHLSItem(seekTo time: TimeInterval, height: Int?) async {
        ...
        itemObserverTask = Task { [weak self] in
            for await status in item.statusStream {
                ...
                case .failed:
                    let err = item.error.map { "\($0)" } ?? "nil"
                    playerLog.error("❌ [wkHLS quality] AVPlayerItem failed: \(err) — clearing stale wkHLS session, routing through 403 recovery")
                    // The variant URLs belong to a short-lived extraction session; a
                    // failed variant item means the session is stale. Without recovery
                    // the failed item stays installed and playback dead-ends on a black
                    // frame. Mirror the muxed-fallback guard in selectFormat: clear the
                    // wkHLS state and force .retry403Recovery (fresh WKWebView
                    // extraction) via the synthesized -1102 error.
                    self.wkHLSMasterURL = nil
                    self.hlsVariantURLs = [:]
                    self.webHLSProxyLoader = nil
                    let recoveryError = NSError(domain: NSURLErrorDomain, code: -1102,
                                                userInfo: [NSLocalizedDescriptionKey: "wkHLS quality-switch item failed: \(err)"])
                    let failedQuality = height.flatMap { AppSettings.VideoQuality.from(height: $0) } ?? .auto
                    await self.delegate?.qualityItemDidFail(
                        error: recoveryError,
