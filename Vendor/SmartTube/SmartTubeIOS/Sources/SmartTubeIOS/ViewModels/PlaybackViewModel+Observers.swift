            ...
            Task { @MainActor [weak self] in
                ...
                if playerWentSilent {
                    ...
                    if recoveryCount >= 3 && isRapidRepeat && self.exhaustiveRetryTask == nil {
                        ...
                        // Tracked (unlike before) so stop()/suspend()/load() can cancel it.
                        // Untracked, this task's !isPlaying/rate==0 guard passes precisely
                        // AFTER stop()/suspend() (both pause and set isPlaying=false), so a
                        // stall within ~2s of dismissing the player resurrected audio on the
                        // parked item with no visible UI (same after-teardown @Observable
                        // mutation class as Firebase a013be1c).
                        let stalledVideoId = self.currentVideo?.id
                        self.stallRecoveryTask?.cancel()
                        self.stallRecoveryTask = Task { @MainActor [weak self] in
                            try? await Task.sleep(nanoseconds: 2_000_000_000)
                            guard let self, !Task.isCancelled,
                                  !self.isPlaying, self.player.rate == 0,
                                  self.currentVideo?.id == stalledVideoId,
                            ...
                            self.player.seek(
                                ...
                                Task { @MainActor [weak self] in
                                    // stallRecoveryTask goes nil/cancelled in stop()/suspend()/
                                    // load() — bail if teardown happened while the seek ran.
                                    guard let self, !self.isPlaying, self.player.rate == 0,
                                          self.stallRecoveryTask?.isCancelled == false else { return }
