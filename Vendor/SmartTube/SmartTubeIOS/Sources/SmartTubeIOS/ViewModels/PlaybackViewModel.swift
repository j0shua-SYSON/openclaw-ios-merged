...
public final class PlaybackViewModel {
    ...
    /// 2s-delayed stall-recovery task spawned by setupRateObserver(). Tracked so
    /// stop()/suspend()/load() can cancel it: its `!isPlaying && rate == 0` guard
    /// passes precisely after stop()/suspend() (both set isPlaying=false and pause),
    /// so an uncancelled task resurrects audio playback on the parked item after
    /// the player UI has been dismissed.
    var stallRecoveryTask: Task<Void, Never>?
...
extension PlaybackViewModel: QualityEventHandler {
    ...
    func qualityItemDidFail(error: Error?, quality: AppSettings.VideoQuality, hasAppliedH264Cap: Bool) async {
        // A failed item can never reach qualityItemDidBecomeReady — the only other
        // place this flag is cleared. Clear it here so the periodic time observer
        // (frozen while true) resumes once a recovery path restores playback; the
        // recovery paths that install a new item set it true again themselves.
        isQualityChangePending = false
