...
extension PlaybackViewModel {
    ...
    func retryWith403Recovery(video: Video, originalError: Error?) async {
        ...
        // The failed quality item never reaches qualityItemDidBecomeReady, and none of
        // exhaustiveRetry's success paths clear this flag — left true it permanently
        // freezes the time observer (scrubber, SponsorBlock skips, caption cues) even
        // though recovered playback continues.
        ...
        do {
            ...
            for await status in compositeItem.statusStream {
                ...
                case .failed:
                    ...
                    // Clear the time-observer freeze set before replaceCurrentItem above;
                    // this failure path never reaches the readyToPlay reset.
                    isQualityChangePending = false
                    selectedFormat = nil
                    if statsForNerdsVisible { updateStatsSnapshot() }
                    if let video = currentVideo {
