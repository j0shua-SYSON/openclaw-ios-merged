...
extension PlaybackViewModel {
    ...
    public func load(video: Video) {
        ...
        // Authoritative per-video reset: if a quality switch was cancelled mid-swap
        // (qualityTask cancelled between replaceCurrentItem and readyToPlay) the flag
        // stays true and would freeze the time observer for this and every following
        // video — frozen scrubber, dead SponsorBlock auto-skip, stuck captions.
        ...
        audioManager.reset()
        endCards = []
        // Per-video state on the shared PlayerFeatures controllers. Without these,
        // the previous video's comments stay pinned (CommentsController's isEmpty
        // guard) and its like/dislike state carries over whenever the new video's
        // nextInfo has no likeStatus — tapping the stale "liked" button then sends
        // removeLike for a video that was never liked.
        comments.reset()
        likeDislike.setLikeStatus(.none)
    ...
    public func suspend() {
        ...
        exhaustiveRetryTask?.cancel()
        exhaustiveRetryTask = nil
        // Same reasoning as stop(): the delayed stall recovery fires exactly when
        // isPlaying=false and rate==0 — the state this suspend() just created —
        // and would resurrect playback after the player UI is gone.
        stallRecoveryTask?.cancel()
        stallRecoveryTask = nil
    ...
    func loadAsync(video: Video) async {
        ...
        setupRateObserver()
        #if canImport(UIKit)
        // Same for the AVAudioSession interruption observer: stop() removes it and
        // (unlike the rate observer and command center) nothing re-registered it, so
        // after one full stop() every later session misdiagnosed phone calls/Siri as
        // playback stalls and never auto-resumed when the interruption ended.
    ...
    public func stop() {
        ...
        exhaustiveRetryTask?.cancel()
        exhaustiveRetryTask = nil
        // Cancel the rate-observer's delayed stall recovery: its !isPlaying/rate==0
        // guard passes after the pause above, so left running it would seek the
        // parked item and set rate/isPlaying back — audible ghost playback with no
        // visible player.
        stallRecoveryTask?.cancel()
        stallRecoveryTask = nil
