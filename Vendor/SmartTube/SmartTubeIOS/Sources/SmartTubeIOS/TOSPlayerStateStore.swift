...
public final class TOSPlayerStateStore {
    ...
    public func play(video: Video, api: InnerTubeAPI) {
        ...
        // Settings and auth normally reach the vm via TOSPlayerView.onAppear, which
        // (see the callback comment below) does NOT re-fire when this store swaps the
        // vm under an already-presented cover. Carry them over from the outgoing vm so
        // swipe-created sessions keep the user's playback settings (speed, SponsorBlock,
        // watch history, autoplay) and their auth binding instead of AppSettings()
        // defaults and anonymous tracking.
        if let outgoing = vm {
            newVM.updateSettings(outgoing.settings)
            newVM.updateAuthToken(outgoing.appliedAuthToken)
            newVM.updateSAPISID(outgoing.appliedSAPISID)
        }
    ...
    public func stop() {
        ...
        presentation = .hidden
        seenVideoIds = []
        // Clear the swipe history too — leaving it populated makes the next session's
        // first play() report hasPrevious=true, so a swipe-right jumps to a leftover
        // video from the previous, closed session.
        history = []
        let action = dismissPlayerAction
        dismissPlayerAction = nil
