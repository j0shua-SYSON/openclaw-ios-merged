...
public final class PlayerStateStore {
    ...
    var playingVideo: Video? {
        // vm.currentVideo, not the store's currentVideo: the store only learns about
        // videos opened through play(video:), but the VM navigates on its own
        // (playNext/playPrevious/autoplay/end-card taps) — after any of those the
        // store's copy is stale and the mini-player would show the previous video's
        // title next to the new video's live frames.
        vm.isLoading ? vm.history.last : (vm.currentVideo ?? currentVideo)
    }
