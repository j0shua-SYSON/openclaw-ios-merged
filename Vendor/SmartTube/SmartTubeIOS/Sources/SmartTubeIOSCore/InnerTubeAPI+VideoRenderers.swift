            ...
            viewCount: {
                ...
                for line in lines.dropFirst() {
                    ...
                    for item in items {
                        ...
                        // Only lines that actually denote a count ("N views" / "N watching",
                        // hl=en is hardcoded): date lines like "3 weeks ago" also contain a
                        // digit and would otherwise become a bogus 3-view count.
                        let lower = str.lowercased()
                        guard lower.contains("view") || lower.contains("watching") else { continue }
            ...
            viewCount: {
                ...
                for row in metaRows.dropFirst() {
                    ...
                    for part in parts {
                        ...
                        // Same guard as parseTileRenderer: date rows ("3 weeks ago")
                        // must not be misread as a 3-view count.
                        let lower = str.lowercased()
                        guard lower.contains("view") || lower.contains("watching") else { continue }
                        if let count = extractNumber(str) { return count }
                    }
