...
public final class BrowseViewModel {
    ...
    public func updateAuthToken(_ token: String?) async {
        ...
        if token != nil && !wasAuthenticated {
            // Signed in — reload everything. Guarded on the sign-in *transition*
            // (mirrors HomeViewModel.updateAuthToken): the proactive OAuth refresh
            // delivers a new token string roughly hourly, and reloading on every
            // refresh wiped videoGroups mid-browse — full-screen spinner, scroll
            // position and all paginated pages lost.
            loadContent(refresh: true, source: "updateAuthToken")
        } else if token == nil, wasAuthenticated {
    ...
    private func fetchNextPage(for section: BrowseSection, autoChainDepth: Int = 0) async {
        // Set + defer BEFORE the token guard: loadMoreIfNeeded sets isLoadingMore
        // synchronously, and if a refresh cleared videoGroups before this task ran,
        // the early return below used to leave the flag stuck true — silently killing
        // pagination in every section for the rest of the session.
        isLoadingMore = true
