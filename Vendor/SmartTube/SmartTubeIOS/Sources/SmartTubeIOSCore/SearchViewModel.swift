...
public final class ChannelViewModel {
    ...
    private func loadAsync(channelId: String) async {
        // Already loaded this channel — preserve the paginated list and scroll
        // position when the view re-appears (back-navigation, player dismiss).
        // Mirrors PlaylistViewModel.load's guard. A different channelId (view
        // reused for another channel) still reloads.
        if channel?.id == channelId, !videos.isEmpty { return }
        isLoading = true
        defer { isLoading = false }
        do {
